...
export class LocationService {
  ...
  private readonly HOTSPOTS: HotspotConfig[] = [
    ...
    { name: 'Ba Đình', lat: 21.0352, lng: 105.819, deviceCount: 6 },
    { name: 'Đống Đa', lat: 21.0136, lng: 105.827, deviceCount: 6 },
  ...
  generateDeviceLocations(
    ...
    const totalHotspotDevices = this.HOTSPOTS.reduce(
      (sum, h) => sum + h.deviceCount,
      0,
    );
    ...
    for (const hotspot of this.HOTSPOTS) {
      const devicesForHotspot = Math.round(
        hotspot.deviceCount * scaleFactor * 0.8,
      );
    ...
    this.logger.log(
      `Generated ${locations.length} device locations (${startIndex} to ${deviceIndex - 1})`,
    );
    return locations;
  }
