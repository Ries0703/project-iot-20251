...
export class DevicesController {
  ...
  async update(
    @Param('id') id: string,
    @Body() body: Partial<SimulatedDevice>,
  ) {
    ...
    if (body.isActive !== undefined) {
      // Status changed
      await this.simulatorService.emitStatus(
        device,
        device.isActive ? 'ONLINE' : 'OFFLINE',
      );
    } else if (device.isActive) {
      // If active and just updated metadata, re-announce
      await this.simulatorService.emitStatus(device, 'ONLINE');
    }
  ...
  async triggerEvent(
    ...
    return {
      success: true,
      message: `Triggered ${body.type} at ${device.name}`,
    };
  }
  ...
  async remove(@Param('id') id: string) {
    const device = await this.deviceRepo.findOneBy({ id });
    if (device) {
      await this.simulatorService.emitStatus(device, 'DELETED');
